resource "yandex_kubernetes_node_group" "my_node_groups" {
  ...
  instance_template {
    ...
    network_interface {
      ...
      security_group_ids = [yandex_vpc_security_group.internal.id, yandex_vpc_security_group.k8s_worker.id]
    }
    ...
    boot_disk {
      type = lookup(each.value, "boot_disc_type", "network-hdd")
      size = lookup(each.value, "boot_disc_size", 64)
    }
